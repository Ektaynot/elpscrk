...
def main():
    ...
    keys=input("Info (Split It)->").replace(" ",";").replace(",",";").split(";")
    num=eval(input("\nLength Of The Pwd: "))
    ...
    if keys:
        ...
        cleared_list=list(dict.fromkeys(final_list).keys()) #Remove Repeated strings from the list
...
t1.start()
t1.join()
#if input("Do You Wanna Print It (Y/N) : ").lower() in ['y','yes']:
#    print final_list
